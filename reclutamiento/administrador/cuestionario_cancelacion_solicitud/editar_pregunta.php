...
<body>
    ...
    <?php
        ...
        $query = "SELECT pregunta FROM cuestionario_cancelacion_solicitud WHERE id_pregunta = $id";
        $result = mysqli_query($con,$query);
        $row = mysqli_fetch_array($result);
        $pregunta = $row[0];

        //Colocar el formulario
        echo "<label>Pregunta <input type=\"text\" name=\"pregunta1\" value = \"$pregunta\"></label><br>";   
    ?>
...
<?php
    if(isset($_POST['editar'])){
		...
		if (mysqli_connect_errno())
		{
            ...
            $query = "UPDATE  cuestionario_cancelacion_solicitud
